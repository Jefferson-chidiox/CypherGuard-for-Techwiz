...
class CompleteCipherGuardGUI(FileHandlerMixin):
    def __init__(self, root):
        self.root = root
        self.root.title("CipherGuard - Advanced Cryptographic Security and Education Platform")
        
        # Make window responsive to screen size
        screen_width = self.root.winfo_screenwidth()
        screen_height = self.root.winfo_screenheight()
        
        # Set window to 90% of screen size with larger maximums for better scrolling visibility
        window_width = min(int(screen_width * 0.90), 1400)
        window_height = min(int(screen_height * 0.90), 900)
        ...
        # Set minimum window size (larger to accommodate content)
        ...
        # Add basic mouse wheel support for better navigation
    ...
    def create_simple_scrollable_frame(self, parent):
        """Create a simple scrollable frame using ScrolledText approach"""
        # Create a regular frame that will contain all content
        # This is much simpler and less prone to causing unresponsiveness
    ...
    def on_mousewheel(self, event):
        """Handle mouse wheel scrolling in a simple way"""
        # This provides basic mouse wheel support without complex canvas implementation
        # The existing ScrolledText widgets will handle their own scrolling
        pass
    ...
    def setup_styles(self):
        ...
        # Configure button styles for compact layout
    ...
    def create_modern_interface(self):
        ...
        # Set window icon using shield emoji (for taskbar)
        try:
            self.root.iconify()
            self.root.deiconify()
        except:
            pass  # Fallback if setting icon fails
            
        # Set window title with shield icon
        self.root.title("🛡️ CipherGuard - Advanced Cryptographic Security Suite")
        ...
        # Title and subtitle (compact) with shield icon
    ...
    def create_file_encryption_tab(self):
        """Create a secure communication tab for files and messages"""
        self.secure_comm_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.secure_comm_frame, text='🛡️ Secure Communication')
        
        # Compact welcome section
        welcome_frame = tk.Frame(self.secure_comm_frame, bg='white', relief='raised', bd=1)
        welcome_frame.pack(fill='x', padx=8, pady=5)
        
        tk.Label(welcome_frame, 
                text="🛡️ Secure Communication Hub",
                font=('Segoe UI', 14, 'bold'),
                bg='white',
                fg=self.colors['primary']).pack(pady=5)
        
        tk.Label(welcome_frame,
                text="Encrypt files or messages",
                font=('Segoe UI', 9),
                bg='white',
        ...
        input_section.pack(fill='x', padx=8, pady=5)
        
        # Radio buttons for input method
        self.input_method = tk.StringVar(value="message")
        method_frame = tk.Frame(input_section)
        method_frame.pack(fill='x', padx=8, pady=5)
        
        tk.Radiobutton(method_frame, text="📝 Message", 
                      variable=self.input_method, value="message",
                      command=self.toggle_input_method,
                      font=('Segoe UI', 9)).pack(side='left', padx=8)
        
        tk.Radiobutton(method_frame, text="📁 File", 
                      variable=self.input_method, value="file",
                      command=self.toggle_input_method,
                      font=('Segoe UI', 9)).pack(side='left', padx=8)
        
        # Compact message composition area
        self.message_section = tk.LabelFrame(self.secure_comm_frame, text="Message Content",
                                            font=('Segoe UI', 10, 'bold'),
                                            fg=self.colors['primary'])
        self.message_section.pack(fill='x', padx=8, pady=5)
        
        tk.Label(self.message_section, text="Enter message:", font=('Segoe UI', 9, 'bold')).pack(anchor='w', padx=8, pady=(5, 2))
        self.message_content = scrolledtext.ScrolledText(self.message_section, height=4, font=('Segoe UI', 9))
        self.message_content.pack(fill='x', padx=8, pady=(0, 5))
        
        # Compact file selection area (initially hidden)
        ...
        file_path_frame.pack(fill='x', padx=8, pady=5)
        
        tk.Label(file_path_frame, text="File:", font=('Segoe UI', 9, 'bold')).pack(anchor='w')
        ...
        file_btn_frame.pack(fill='x', padx=8, pady=5)
        
        ttk.Button(file_btn_frame, text="📁 Select File", 
                  command=self.select_file,
                  style='Primary.TButton').pack(side='left', padx=2)
        
        # File type description
        tk.Label(self.file_section, 
                text="Formats: .txt, .eml, .enc",
                font=('Segoe UI', 8),
                fg=self.colors['dark']).pack(padx=8, pady=(0, 5))
        
        # Compact key management section
        key_section = tk.LabelFrame(self.secure_comm_frame, text="Key Management",
                                   font=('Segoe UI', 10, 'bold'),
                                   fg=self.colors['primary'])
        key_section.pack(fill='x', padx=8, pady=5)
        
        # Key text area
        key_text_frame = tk.Frame(key_section)
        key_text_frame.pack(fill='x', padx=8, pady=3)
        
        tk.Label(key_text_frame, text="Key:", font=('Segoe UI', 9, 'bold')).pack(anchor='w')
        self.key_text_area = scrolledtext.ScrolledText(key_text_frame, height=2, font=('Consolas', 8))
        self.key_text_area.pack(fill='x', pady=(2, 0))
        
        # Key buttons
        key_btn_frame = tk.Frame(key_section)
        key_btn_frame.pack(fill='x', padx=8, pady=5)
        
        ttk.Button(key_btn_frame, text="🔑 Generate",
                  command=self.generate_and_display_key,
                  style='Success.TButton').pack(side='left', padx=2)
        
        ttk.Button(key_btn_frame, text="💾 Save",
                  command=self.save_key_to_file,
                  style='Primary.TButton').pack(side='left', padx=2)
        
        ttk.Button(key_btn_frame, text="📥 Load",
                  command=self.load_key_from_file,
                  style='Primary.TButton').pack(side='left', padx=2)
        
        # Compact operations section
        ops_section = tk.LabelFrame(self.secure_comm_frame, text="Operations",
                                   font=('Segoe UI', 10, 'bold'),
                                   fg=self.colors['primary'])
        ops_section.pack(fill='x', padx=8, pady=5)
        
        ops_btn_frame = tk.Frame(ops_section)
        ops_btn_frame.pack(fill='x', padx=8, pady=5)
        
        ttk.Button(ops_btn_frame, text="🔒 Encrypt",
                  command=self.encrypt_and_save_as,
                  style='Success.TButton').pack(side='left', padx=2)
        
        ttk.Button(ops_btn_frame, text="🔓 Decrypt",
        ...
        self.secure_progress.pack(fill='x', padx=8, pady=3)
        
        # Compact results display
        results_section = tk.LabelFrame(self.secure_comm_frame, text="Results",
                                       font=('Segoe UI', 10, 'bold'),
                                       fg=self.colors['primary'])
        results_section.pack(fill='both', expand=True, padx=8, pady=5)
        ...
        self.secure_results.pack(fill='both', expand=True, padx=8, pady=5)
        
        # Add compact initial help text
        self.secure_results.insert('end', "🛡️ CipherGuard Secure Communication Hub\n")
        self.secure_results.insert('end', "=" * 35 + "\n\n")
        self.secure_results.insert('end', "📋 Quick Guide:\n")
        self.secure_results.insert('end', "1. Choose: Message or File\n")
        self.secure_results.insert('end', "2. Generate/Enter key\n")
        self.secure_results.insert('end', "3. Encrypt or Decrypt\n\n")
        self.secure_results.insert('end', "✨ Ready!\n\n")
        ...
        # Add compact initial help text to key area
        self.key_text_area.insert('1.0', "Click 'Generate' or paste key here...")
...
🔐 Secure communication (files & messages)
...
Ready to explore cryptographic security?
    ...
    def show_tutorial_step(self):
        ...
        tutorial_steps = [
            ...
            },
            {
                ...
                'message': 'Now click "🔒 Encrypt & Save" to secure your content in an encrypted format.',
                'tab': 0
            },
            {
    ...
    # Handler setter methods (updated for uni interface)
    ...
    def generate_and_display_key(self):
        ...
        try:
            ...
            self.secure_results.insert('end', "🔑 New key generated!\n")
            self.secure_results.insert('end', "💡 Save key for later use.\n\n")
    ...
    def save_key_to_file(self):
        ...
        try:
            ...
            if filename:
                ...
                self.secure_results.insert('end', f"💾 Key saved: {os.path.basename(filename)}\n")
                self.secure_results.insert('end', "Keep file secure!\n\n")
                self.secure_results.see('end')
    ...
    def load_key_from_file(self):
        ...
        try:
            ...
            if filename:
                ...
                self.secure_results.insert('end', f"📥 Key loaded: {os.path.basename(filename)}\n")
                self.secure_results.insert('end', "Ready for operations.\n\n")
                self.secure_results.see('end')
    ...
    # Digital signature methods for document management
        ...
        if self._run_comparison:
            try:
                ...
                if fig:
                    ...
                    performance_overhead = [25, 85]  
                    
                    confidentiality = [95, 40]
...
"""
    ...
    def toggle_input_method(self):
        ...
        if self.input_method.get() == "message":
            ...
            self.secure_results.insert('end', "📝 Message Mode\n")
            self.secure_results.insert('end', "=" * 20 + "\n\n")
            self.secure_results.insert('end', "Type message, generate key, encrypt.\n\n")
        else:
            ...
            self.secure_results.delete('1.0', 'end')
            self.secure_results.insert('end', "📁 File Mode\n")
            self.secure_results.insert('end', "=" * 20 + "\n\n")
            self.secure_results.insert('end', "Select file, generate key, encrypt.\n\n")
    ...
    def encrypt_and_save(self):
        """Encrypt content (message or file) and save"""
        try:
            ...
            try:
                ...
                self.update_status("Content encrypted successfully")
    ...
    def decrypt_and_save_as(self):
        ...
        try:
            ...
            if not hasattr(self, 'last_encrypted_result') or not self.last_encrypted_result:
                ...
                if (hasattr(self, 'selected_file_path') and 
                    self.selected_file_path and 
                    self.selected_file_path.endswith('.enc')):
                    try:
                        import json
                    ...
                    messagebox.showwarning(
                        ...
                        "There are no messages or files ready for decryption.\n\n" +
                        "To decrypt content, you need to either:\n" +
                        "• First encrypt a message or file in this session, then decrypt it\n" +
                        "• Select an encrypted (.enc) file using 'Upload File' option\n\n" +
                        "Please encrypt some content first or select an .enc file to decrypt."
                    )
                    return
